...
__dtwclust__ package used is created by  Alexis Sarda-Espinosa and is available at [CRAN](https://cran.r-project.org/web/packages/dtwclust/). 

## 2.3. Visual Analytics for spatiotemporal clustering
Based on Ali, Alqahtani, Jones, and Xie's [-@ali_2019_clustering] findings, there is an increasing trend of usage of line plots, geographic maps, heat maps, histograms, and graphs for time series data. Visual analytics is beneficial because it facilitates the collaboration between machine and human capabilities to explore, analyse, understand, and gain insights from data [@ali_2019_clustering]. 

As the results of machine learning depends on the hyperparameter in the setup, a lot of effort and time goes into iteratively trying and evaluating the results of using different parameters [@li_hypertuner]. Interactive visual analytics provides a way to quickly try and evaluate different parameters, empowering the users to get better results in a shorter time [@ali_2019_clustering; @li_hypertuner]. 

## 2.4. Gap in current practices of spatiotemporal clustering



## 2.3. R Packages

The packages that we are going to use in this analysis are: 
...
packages = c("tidyverse","timetk", "lubridate","zoo","sf", "tmap","spdep","dtwclust", "dplyr","plotly", "ggthemes","dendextend","ggdendro", "RColorBrewer",  "DT","egg")
for (p in packages){
...
We want to see whether the data resembles normal distribution 
```{r}

crime_major_hist <- ggplot(crime_major)+
  geom_histogram(aes(Count))+
  facet_wrap(~`Major Category`,scales="free")

crime_rate_hist <- ggplot(crime_rate)+
  geom_histogram(aes(Crime_Rate))+
  facet_wrap(~`Major Category`,scales="free")

grid.arrange(crime_major_hist,crime_rate_hist)
```


## 3.4. Geospatial Data Exploration
We can use a map to see the distribution of crime records in different boroughs. 
To do so, we need the geospatial data that contains the polygon for each borough. The data is also obtained from London Datastore [-@londondatastore_2021_statistical]. Using ```st_read()``` function, the file will be read as a simple feature format.
...
<!-- However, we can see that the range of the crime count is still highly skewed. The top quantil for average crime rate has 3,281 to 11,356 crime recorded/1 million population. We can further smooth the data by normalizing with the neighbour rate [@medina_2019_chapter]. -->

<!-- ```{r} -->
<!-- crime_map_sp <- as(crime_map, "Spatial") -->
<!-- neighbour <- poly2nb(crime_map_sp, row.names=crime_map_sp$GSS_Code) -->

<!-- length(crime_map$AvgCount) -->

<!-- length(neighbour) -->
<!-- eb2 <- EBlocal(crime_map$AvgCount, crime_map$AvgPopulation, neighbour) -->
<!-- crime_map$AvgSmoothRate <- eb2$est * 100000 -->
<!-- ``` -->
...
We will now examine the arguments that can be passed to the function to improve the results of the clustering. Some of the arguments that we can expose to the useres are:

Arguments | Description
---------|-----------
series | Data in a form of list of series, numeric matrix, or a data frame where each observation is in a a row and the columns shows the time period. Can allow the users to choose the geographical aggregation level or the crime aggregation level.
type | Type of clustering method to use. ["partitional", "hierarchical", "tadpole", "fuzzy"]
k | Number of clusters
preproc | function to pre-process the data, deafult is zscore() if centroid = "shape".
distance | used with partitional, hierarchical, and fuzzy clustering. ["dtw", "dtw2", "dtw_basic", "dtw_lb", "lbk", "sbd", "gak", "sdtw"]
...
dtw_lb, lbk will not be used because it need special argument window.size.

There are more arguments that can be used to control the preprocessing, distance, and centroid functions, but they are too granular and each function may require different arguments. Therefore, we will only use these arguments in the Shiny app. 

## 4.1. Partitional & Fuzzy Dynamic Time Warping Clustering

To get the number of desired clusters, we will show the silhoutte plot for various number of clusters, which can be computed using ```cvi()``` function in __dtwclust__. This package also has a function called ```compare_clusterings()``` which can automatically select the best performing parameters and returns the best configurations, however we want to give the users freedom in choosing the 
...
![Figure 3. dendextend set() arguments [@a2020_beautiful]](./images/dendextend_arguments.png)
